...
class RampShape(LoadTestShape):
    X = 600            
    ramp_time = 180    #3 minutos para subir
    hold_time = 300    #5 minutos de espera
    ...
    def tick(self):
        ...
        if run_time < self.ramp_time:
            user_count = int((run_time / self.ramp_time) * self.X)
            return (user_count, user_count / 10)  

        elif run_time < self.total_time:
            return (self.X, self.X / 10)
